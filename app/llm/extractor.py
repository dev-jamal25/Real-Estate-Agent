...
class Stage1Extractor:
    ...
    def __init__(
        ...
        Args:
            llm_client: LLM client instance (uses mock by default)
    ...
    def extract(
        ...
        """
        Extract features from the latest user message.
        
        IMPORTANT: assistant_message is generated by code (not the LLM).
        - For incomplete extractions: code generates a follow-up question asking for missing fields
        - For complete extractions: code generates a completion message
        
        The LLM's assistant_message in the JSON is parsed but always overridden.
        This ensures consistent, user-friendly follow-up wording across all turns.
        
        Args:
            latest_user_message: The latest message from the user
            current_known_feature_state: Previously known features (from earlier turns or overrides)
        
        Returns:
            Stage1ExtractionResult with extracted features, missing list, and code-generated follow-up message
        
        Raises:
            ValueError: If the LLM response is invalid JSON or missing required fields
        """
        # Prepare context
    ...
    def _normalize_extracted_features(self, raw_features: dict) -> dict:
        ...
        Applies validation and type coercion to each extracted feature.
        Drops invalid features silently (they won't be included in the result).
        
        Args:
            raw_features: Raw features from LLM response
        
        Returns:
            Normalized features dict with valid values only
        """
        normalized = {}
